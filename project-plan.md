...
**Current diagnosis, as of this measurement:** recall is now essentially solved — only 10/200 sessions miss entirely (Hit Rate@10 0.950), and nearly all of those still have the target somewhere in the candidate pool. The remaining lever is **rank-1 precision** (drives MRR, 30% of TechnicalScore): only ~53% of hits land at rank 1 overall, and Buying specifically sits at ~43% despite being the scenario with the most disclosed information per session — worse than Browsing (56%) and much worse than Intent Override (77%). The category-route fix above helped hit rate and MRR broadly but barely moved Buying's rank-1 rate specifically, meaning whatever's suppressing Buying's precision is a separate, still-open problem — worth investigating with concrete failing-session transcripts (see the diagnostic pattern used to find the category bug) rather than guessing at another global weight tweak.
